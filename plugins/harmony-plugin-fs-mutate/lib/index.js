/**
 * Function plugin registering the model-facing `delete` and `move` filesystem
 * tools. Both route every mutation through `ctx.fs` — the seam that owns target
 * identity, atomicity, and the sandbox fence — so this plugin never opens a path
 * itself. Named exports only: a default export would make the Loader discard
 * `inject`.
...
/** Services required before either tool registers. */
export const inject = ['tools', 'fs']

/**
 * Default per-file byte cap for the `move` copy. Mirrors `dsh-fs-local`'s
 * `diffBasisMaxBytes` default, the existing precedent for how much text one
 * whole-file operation buffers; `readBytes` makes a cap mandatory so a backend
 * never buffers an unbounded file.
 */
...
/**
 * Register both tools over the mounted filesystem.
