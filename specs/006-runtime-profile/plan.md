...
| `patches/dsh-symlink-to-copy.patch` | symlink EACCES → cpSync 回退 | 56 行 / 2515B |
| `patches/dsh-allow-all-interfaces.patch` | 移除 `0.0.0.0` 拒绝检查 | 14 行 / 771B |
| `patches/dsh-disable-hmr.patch` | `DSH_DISABLE_HMR` 开关跳过 HMR | 14 行 / 663B |
| `patches/dsh-disable-native-picker.patch` | Electron 下目录选择器走 browse | 14 行 / 1044B |
...
| FR-006-008（symlink 回退 copy） | `patches/dsh-symlink-to-copy.patch`（`ensureSymlink` 的 cpSync 回退 + isDirectory return） |
| FR-006-009（移除 0.0.0.0 拒绝） | `patches/dsh-allow-all-interfaces.patch`（删除 `program.error(...)` 3 行） |
| FR-006-010（DSH_DISABLE_HMR） | `patches/dsh-disable-hmr.patch`（`!process.env.DSH_DISABLE_HMR &&`） |
| FR-006-011（Electron → browse） | `patches/dsh-disable-native-picker.patch`（`if ('electron' in process.versions) return 'browse'`） |
