...
test('the no-op chain members keep their previous behaviour', async () => {
  const image = esm(RGBA_PNG);
  assert.equal((await image.raw().toBuffer()).length, 0);
  assert.equal((await image.toBuffer()).length, 0);
  assert.equal(await image.toFile('/tmp/unused'), undefined);
  assert.deepEqual(await image.stats(), { channels: 3 });
